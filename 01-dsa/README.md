# DSA Progress Tracker (Google L4)

> Each topic has 3 columns: **Done**, **In Progress**, **Todo**. Update as you go. Solve problems in your chosen language.

## Phase 1 — Foundations (Weeks 1-4)

| # | Topic | Done | In Progress | Todo |
|---|---|---|---|---|
| 1 | Arrays & Hashing | ☐ | ☐ | ☐ |
| 2 | Two Pointers | ☐ | ☐ | ☐ |
| 3 | Sliding Window | ☐ | ☐ | ☐ |
| 4 | Stack | ☐ | ☐ | ☐ |
| 5 | Binary Search | ☐ | ☐ | ☐ |
| 6 | Linked List | ☐ | ☐ | ☐ |

## Phase 2 — Core (Weeks 5-10)

| # | Topic | Done | In Progress | Todo |
|---|---|---|---|---|
| 7 | Trees (BFS/DFS) | ☐ | ☐ | ☐ |
| 8 | BST | ☐ | ☐ | ☐ |
| 9 | Tries | ☐ | ☐ | ☐ |
| 10 | Heap / Priority Queue | ☐ | ☐ | ☐ |
| 11 | Backtracking | ☐ | ☐ | ☐ |
| 12 | Graphs (BFS/DFS/Topo) | ☐ | ☐ | ☐ |

## Phase 3 — Advanced (Weeks 11-16)

| # | Topic | Done | In Progress | Todo |
|---|---|---|---|---|
| 13 | Greedy | ☐ | ☐ | ☐ |
| 14 | Dynamic Programming (1D) | ☐ | ☐ | ☐ |
| 15 | Dynamic Programming (2D) | ☐ | ☐ | ☐ |
| 16 | Intervals | ☐ | ☐ | ☐ |
| 17 | Bit Manipulation | ☐ | ☐ | ☐ |
| 18 | Math & Geometry | ☐ | ☐ | ☐ |


## How I approach a problem
1. Read problem, restate it in my own words (2 min)
2. Think of brute force, mention trade-offs (3 min)
3. Identify pattern, propose optimal (5 min)
4. Code cleanly (15-20 min)
5. Test with edge cases (5 min)

## Resources I'm using
- (Fill in: LeetCode/NeetCode 150? Blind 75? AlgoExpert?)

## Daily target
- 2-3 problems/day on weekdays, 4-5 on weekends
- Pattern-first, not problem-first
